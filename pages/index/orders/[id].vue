                    ...
                    <template #bodyCell="{ record, column }">
                        <template v-if="column.key === 'book'">
                            <div class="flex items-center gap-2">
                                <NuxtImg :src="record.book?.cover_image" :alt="record.book?.title" width="100"
                                    height="100" />
                                <div>
                                    <span class="block mb-3">{{ record.book?.title }}</span>
                                    <span class="text-gray-500">Số lượng còn lại: {{ record.book.quantity }}</span>
                                </div>
                        ...
                        <template v-if="column.key === 'action'">
                            <a-button type="primary" @click="navigateTo('/books/' + record.book_id)">Xem chi
                                tiết</a-button>
                        </template>
                    </template>
...
const query = reactive({
    'with[]': ['user', 'orderItems', 'orderItems.book'],
});
