<template>
    <div class="px-6 py-10">
        ...
        <a-table :columns="columns" :dataSource="books?.data ?? []" rowKey="id" bordered :pagination="false" >
            <template #bodyCell="{ column, record }">
                <template v-if="column.dataIndex === 'authors'">
                    <ul v-if="record.authors && record.authors.length > 0" v-for="(author, index) in record.authors"
                        :key="index">
                        <li class="text-nowrap font-medium"> - {{ author.name }}</li>
                    </ul>
                    <span v-else class="text-red-500">Chưa có tác giả</span>
                </template>
                ...
                <template v-if="column.dataIndex === 'action'">
                    <div class="flex gap-2">
                        <a-button type="primary" @click="navigateTo('/books/' + record.id)">Xem Chi tiết</a-button>
                        <a-button @click="navigateTo('/books/' + record.id + '/edit')">Sửa</a-button>
                        <a-popconfirm
            ...
            <template #title>
                <div class="mb-4">
                    <a-button type="primary" @click="navigateTo('/books/create')">Thêm mới</a-button>
                    <div class="my-4 flex items-center gap-2">
                        <h6 class=" text-base
                            font-medium">Danh sách sách</h6>
                    ...
                    <div class="flex gap-x-10">
                        <div class="space-x-6">
                            <a-input-search v-model:value="search"
                                placeholder="Tìm kiếm theo tên sách hoặc nhà xuất bản..." style="width: 350px"
                                enter-button @search="() => {
                                    loading = true;
                                    console.log(search);
                        ...
                        <div class="flex gap-2 items-center">
                            ...
                            <a-select v-model:value="query.sort" style="width: 200px" placeholder="Sắp xếp">
                                ...
                                <a-select-option value="author_null">Sách chưa có tác giả</a-select-option>
                                <a-select-option value="publisher_null">Sách chưa có nhà xuất bản</a-select-option>
                            </a-select>
                        </div>
            ...
            <template #footer>
                <div class="flex justify-end w-full">
                    <a-pagination v-model:current="query.page" :total="books?.total"
