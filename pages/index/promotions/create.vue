<template>
  <div class="px-10 py-10">
    ...
    <div class="flex flex-nowrap gap-10">
      ...
      <div class="w-1/2">
        <h6 class="mb-6">Chọn sách áp dụng cho chương trình: </h6>
        <div class="flex gap-10 mb-6">
                    <a-input v-model:value="allBookSearch" placeholder="Tìm kiếm sách" />
                    <a-button type="primary" @click="handleSearchTitleAllBook">Tìm kiếm</a-button>
                </div>
        <a-checkbox-group v-model:value="formCreate.items" class="w-full">
          <div ref="refScrollContainer" class="max-h-[40vh] overflow-y-scroll w-full border p-4" @scroll="handleScroll">
            <div v-for="book in allBooks" :key="book.id" class="flex w-full gap-4 my-2">
              <a-checkbox :value="book.id">

              </a-checkbox>
              <div class="w-full">
                <div class="flex items-center gap-2">
                  <NuxtImg :src="book.cover_image" class="mr-2 rounded-full h-12 w-12" />
                  <div>
                    <span>{{ book.title }}</span>
                    <p class="text-gray-500">{{ book?.authors?.map(author => author.name).join(', ') }}</p>
                    <p class="text-gray-500">Giá gốc: {{ formatCurrency(book.price) }}</p>
                    <p class="text-gray-500">Giảm giá hiện tại: {{ book.discount }}%</p>
        ...
        <h6 class="my-4">Sách được chọn áp dụng khuyến mãi:</h6>
        <!--Sách được chọn áp dụng khuyến mãi theo formDiscount-->
        <div v-if="formCreate.items.length > 0" class="mt-4 border p-4 max-h-[40vh] overflow-y-scroll">
          <div v-for="book in bookApplyPromotion" :key="book.id" 
            ...
            <div class="w-full">
              <div class="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p class="text-gray-500">Giá bán hiện tại: {{ formatCurrency(book.price * (100 - formCreate.discount )/100) }}%</p>
                </div>
              </div>
            </div>
            <Icon name="i-material-symbols-delete-outline-rounded" class="cursor-pointer text-3xl" @click="handleDeleteBookApplyPromotion(book.id)"/>
          </div>
        </div>
        <div v-else class="border p-4">
          <a-empty description="Chưa có sách nào được chọn"></a-empty>
        </div>
...
const allBookSearch = ref('')

const handleSearchTitleAllBook = () => {
    bookQuery.search.title = allBookSearch.value;

}

const allBooksAfterSearch = ref<IBook[]>([]);

const { data: books, refresh: refreshBooks } = await useFetch<IResponsePagination<IBook>>('api/books', {
    baseURL: useRuntimeConfig().public.baseURLAPI,
    headers: {
        Authorization: `Bearer ${accessToken.value}`
    },
    ...
    onResponse: ({ response }) => {
        if (response.ok) {
            allBooks.value = [...allBooks.value, ...(response._data.data ?? [])];
            if(bookQuery.search.title != ''){
                allBooksAfterSearch.value = response._data.data ?? [];
            }
        }
    }
});
...
const handleScroll = () => {
  if (refScrollContainer.value) {
    ...
    if (scrollTop + clientHeight >= scrollHeight) {
      ...
      let max = Math.ceil((books?.value?.total ?? 0) / (books?.value?.per_page ?? 1));
      console.log('max', max);
      if (bookQuery.page < max) {
        bookQuery.page++;
      }
      console.log('bookQuery.page', bookQuery.page);
...
const bookApplyPromotion = computed(() => {
  return allBooks.value.filter(book => formCreate.items.includes(book.id));
});

const handleDeleteBookApplyPromotion = (id: number) => {
