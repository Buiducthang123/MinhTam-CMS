            ...
            <div class="w-1/2">
                ...
                <a-checkbox-group v-model:value="formEdit.items" class="w-full">
                    <div ref="refScrollContainer" class="max-h-[40vh] overflow-y-scroll w-full border p-4"
                        @scroll="handleScroll">
                        <div v-for="book in (bookQuery.search.title != ''? allBooksAfterSearch : allBooks)" :key="book.id" class="flex w-full gap-4 my-2">
                            ...
                            <div class="w-full">
                                <div class="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-500">{{ book?.authors?.map(author => author.name).join(', ')
                                            }}</p>
                ...
                <div v-if="formEdit.items.length > 0" class="mt-4 border p-4 max-h-[40vh] overflow-y-scroll">
                    <div v-for="book in bookApplyPromotion" :key="book.id" class="flex w-full gap-4 my-2">
                        <div class="w-full">
                            <div class="flex items-center gap-2">
                                <NuxtImg :src="book.cover_image" class="mr-2 rounded-full h-12 w-12" />
                                <div>
                                    ...
                                    <p class="text-gray-500">Giá bán hiện tại: {{ formatCurrency(book.price * (100 -
                                        formEdit.discount) / 100) }}</p>
                                </div>
                            </div>
                        </div>
                        <Icon name="i-material-symbols-delete-outline-rounded" class="cursor-pointer text-3xl"
                            @click="handleDeleteBookApplyPromotion(book.id)" />
                    </div>
                </div>
...
const bookApplyPromotion = computed(() => {
    return allBooks.value.filter(book => formEdit.items.includes(book.id));
});
